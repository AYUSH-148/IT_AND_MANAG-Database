...
const Tabs = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      const res = await fetch(`${import.meta.env.API}/api/colleges/all?searchTerm=${activeTab}`);
  ...
  const handleShowMore = async () => {
    ...
    try {
      const res = await fetch(`${import.meta.env.API}/api/colleges/all?startIndex=${startIndex}`);
      const data = await res.json();
      if (res.ok) {
